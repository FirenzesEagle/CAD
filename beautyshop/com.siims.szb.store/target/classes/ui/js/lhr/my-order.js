...
$(document).ready(function(){
	...
	$('.order-type').delegate("li","click",function(){
		...
		if(type==1){
			...
			$('.mainorder-good').show();
			$('.mainorder-good').find(".state-list .state-list-data ").each(function(){
				$(this).removeClass("stateChoosed");
			});
			$('.mainorder-good').find(".state-list").find("li:first").addClass("stateChoosed");
			creatGoodOrder("create");
			//creatGoodOrder("create");
			window.location.reload();
		}else if(type==2){
			$('.mainorder-good').hide();
			$('.mainorder-service').show();
			$('.mainorder-service').find(".state-list .state-list-data ").each(function(){
				$(this).removeClass("stateChoosed");
			});
			$('.mainorder-service').find(".state-list").find("li:first").addClass("stateChoosed");
			creatServiceOrder(1);
		}
	});
	...
	/*$('.mainorder-service').delegate(".state-list-data","click",function(){
		$(this).parent().children().each(function(){
			$(this).removeClass("stateChoosed");
		});
		$(this).addClass("stateChoosed");
	...
	/*商品订单中的按钮操作*/
	$('.mainorder-good').delegate(".bt-pay","click",function(){
		var orderid=$(this).parents(".order-list-detail").attr("id");
		//var ordernum=$('#'+orderid).find('.orderNumber').text();
		var money=parseFloat($('#'+orderid).find('.priceval').text());
		var returnurl="http://182.92.4.200:8838/siims/szb/order/begin.jspx?openid="+openid;
		var notifyurl='http://182.92.4.200:8838/siims/szb/order/updateStatus.jspx?id='+orderid+'&status=payed';
		//支付方式
		$('#'+orderid).find('.good-list').each(function(){
...
var goodlist='<li class="good-list" data-paytype="PAYTYPE" ><div class="good-list-img"><img src="GOODPICURL" alt="" /></div>'
						...
						success:function (data) {
							...
							if(data.result=="success"){
								...
								$.each(data.list,function(i){ 
									...
									creatStateButton(data.list[i],0);	
						...
						success:function (data) {
							...
							if(data.result=="success"){
								...
								$.each(data.list,function(i){ 
									...
									var temp  = goodlist.replace("PAYTYPE", data.list[i].pay_type).replace("GOODPICURL", data.list[i].goodPicUrl).replace("GOODCONFIG",data.list[i].goodsConfig).replace("GOODNAME",data.list[i].goodsName);
								...
								$(orderid).find('.numberval').text(num);
...
function creatStateButton(data,num){
	...
	switch (data.status){
		case "create":
				...
				html+='<p>小计：&yen;<span class="totalNum priceval">'+data.price+'</span></p>';
				html+='<p>数量：<span class="totalNum numberval">'+num+'</span>件</p>';
