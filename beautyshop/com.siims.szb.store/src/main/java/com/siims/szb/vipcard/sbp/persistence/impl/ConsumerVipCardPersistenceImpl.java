...
public class ConsumerVipCardPersistenceImpl implements
	...
	*/
	...
	public String addConsumerVipCardData(ConsumerVipCardData data) {
		...
		String hql = "from ConsumerVipCardData data where data.vipCardTypeId = '" + data.getVipCardTypeId() + "'";
		List<ConsumerVipCardData> list = hibernateDao.queryList(hql, null);
	...
	/**
	 * 根据会员的openid和店铺的shopid来查询拥有的会员卡
	 * @author libo <br/>
	 * 2015-10-10
	 */
	public List<ConsumerVipCardData> queryConsumerVipCardDataByOpenIdAndShopId(String openid, String shopid) {
		String hql = "from ConsumerVipCardData data where data.consumerWeixinOpenId='"+openid 
				+"' AND data.isDelete=0 AND data.ispay=1 AND data.vipCardTypeId in (select type.id from VipCardTypeData type where type.storeUserId = '"+shopid+"')";
		List<ConsumerVipCardData> list = hibernateDao.queryList(hql, null);
		return list;
	}
